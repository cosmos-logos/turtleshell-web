...
export function OceanConnectPanel({ ocean, config, onClose }: Props) {
  ...
  const rgb = hexToRgb(ocean.oceanColor);
  ...
  const handleConnect = async (mode: Mode) => {
    ...
    try {
      ...
      const displayName = mode === 'cloud' ? `${ocean.name} (Olympus-Grid)`
        : mode === 'dev' ? (devName || `${ocean.name} (Developer)`)
        : `${ocean.name} (Off-Grid)`;
  ...
  return createPortal(
    <div className="fixed inset-0 z-50 flex justify-end" onClick={onClose}>
      ...
      <div
        ...
        style={{
          background: '#060c18',
          borderLeft: `1px solid rgba(${rgb}, 0.1)`,
      ...
      >
        {/* Header */}
        <div className="sticky top-0 z-10 p-6 pb-4 flex items-center justify-between" style={{ background: '#060c18', borderBottom: `1px solid rgba(${rgb}, 0.08)` }}>
          <div className="flex items-center gap-3">
            <span className="text-3xl">{ocean.creature}</span>
            <div>
              <h2 className="font-cinzel text-xl font-semibold" style={{ color: '#c0e0f8' }}>{ocean.name}</h2>
              <div className="text-2xs uppercase tracking-[0.2em]" style={{ color: ocean.oceanColor }}>{ocean.creatureName}</div>
            </div>
          </div>
          <button onClick={onClose} className="p-2 rounded-lg hover:bg-white/5 transition-colors">
            <X size={18} style={{ color: '#4080a0' }} />
          </button>
        </div>

        {/* Error */}
        {error && (
          <div className="mx-6 mt-4 px-3 py-2 rounded-lg text-xs text-red-400" style={{ background: 'rgba(220,40,40,0.1)', border: '1px solid rgba(220,40,40,0.2)' }}>
            {error}
          </div>
        )}
        ...
        <div className="p-6 space-y-4">
          <div className="text-xs uppercase tracking-[0.2em] mb-2" style={{ color: '#1a4060' }}>
            Choose how to run {ocean.name}:
          </div>
          ...
          {modeRows.map(({ mode, icon, label, desc }) => {
            ...
            return (
              <div key={mode} className="rounded-xl p-4" style={{ background: 'rgba(255,255,255,0.02)', border: `1px solid rgba(${rgb}, 0.06)` }}>
                <div className="flex items-center gap-2 mb-2">
                  <span className="text-sm">{icon}</span>
                  <span className="text-xs font-semibold" style={{ color: ocean.oceanColor }}>{label}</span>
                  {connected && <CheckCircle size={12} className="text-green-500 ml-auto" />}
                </div>
                <div className="text-2xs mb-3" style={{ color: '#4080a0' }}>{desc}</div>
                    ...
                    {mode === 'dev' && (
                      <>
                        <input
                          ...
                          placeholder={`${ocean.name} (Developer)`}
                          className="w-full px-3 py-1.5 rounded-md text-xs bg-transparent outline-none"
                          style={{ border: `1px solid rgba(${rgb}, 0.1)`, color: '#c0e0f8' }}
                        />
                        <input
                          value={devUrl}
