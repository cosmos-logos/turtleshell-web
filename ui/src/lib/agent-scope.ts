...
export function useActiveAgentScope(): AgentScope {
    ...
    if (cosmosAgent) {
        const codename = cosmosAgent.manifest.identity.codename;
        if (isAthenaFamily(codename)) {
            return {
                displayName: 'Athena',
                avatar: '🐙',
                color: cosmosAgent.manifest.display?.color ?? '#a87ef0',
        ...
        const name = agentDisplayName(cosmosAgent);
        return {
            displayName: name,
            avatar: name.charAt(0).toUpperCase(),
            color: cosmosAgent.manifest.display?.color ?? '#6366f1',
            greeting: DEFAULT_GREETING,
            cta: DEFAULT_CTA,
            matches: (id) => id === cosmosAgent.id,
            toQueryParams: () => {
                const p = new URLSearchParams();
                p.set('agentIds', cosmosAgent.id);
                return p;
            },
            kind: 'cosmos',
        };
    ...
    const id = builtin.id;
    const greeting = id === 'cosmos' ? COSMOS_GREETING
        : id === 'logos' ? LOGOS_GREETING
        : DEFAULT_GREETING;
    const cta = id === 'cosmos' ? COSMOS_CTA
        : id === 'logos' ? LOGOS_CTA
        : DEFAULT_CTA;
    return {
        displayName: builtin.name,
        avatar: builtin.icon || builtin.name.charAt(0).toUpperCase(),
        color: '#6366f1',
        greeting,
        cta,
        matches: (agentId) => agentId === id,
        toQueryParams: () => {
            const p = new URLSearchParams();
            p.set('agentIds', id);
            return p;
